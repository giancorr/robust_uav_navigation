...
public:
    SprayTargetHeuristic() : Node("spray_heuristic_node")
    {
        ...
        // Sottoscrizione allo stato della FSM per sapere se il drone può accettare ordini
        ...
        sub_odom_ = this->create_subscription<px4_msgs::msg::VehicleOdometry>(
            ...
            [this](const px4_msgs::msg::VehicleOdometry::SharedPtr msg) {
                // In NED, North (X) is actually forward, but we receive ENU position via PX4 if it's not relay?
                // Wait, PX4 vehicle_odometry out is in NED. 
                // So msg->position[0] is X (North, forward).
...
private:
    ...
    void health_callback(const std_msgs::msg::String::SharedPtr msg) {
        if (msg->data == "INCONSISTENT" || msg->data == "POTENTIALLY_INCONSISTENT") {
            if (!vio_in_danger_) {
                RCLCPP_WARN(this->get_logger(), "PERICOLO VIO RILEVATO: %s", msg->data.c_str());
            }
            vio_in_danger_ = true;
        } 
        else if (msg->data == "CONSISTENT" || msg->data == "POTENTIALLY_CONSISTENT") {
            if (vio_in_danger_) {
                RCLCPP_INFO(this->get_logger(), "VIO tornato sicuro: %s", msg->data.c_str());
            }
            vio_in_danger_ = false;
        }
        else {
            RCLCPP_WARN_ONCE(this->get_logger(), "Ricevuto stato VIO sconosciuto: %s", msg->data.c_str());
        }
    }

    void feature_callback(const vio_health_monitor::msg::FeatureCount::SharedPtr msg) {
        feat_left_ = msg->left_count;
        feat_right_ = msg->right_count;
    }

    void surface_callback(const vio_health_monitor::msg::SurfaceInfo::SharedPtr msg) {
    ...
    void evaluate_situation() {
        ...
        if (vio_in_danger_ || distance_trigger) {
            ...
            // Non inviare ordini se la FSM non è in NAVIGATING:
            // verrebbero ignorati e sprecheremmo il cooldown
            ...
            RCLCPP_INFO(this->get_logger(), "Valutazione muri - Dist L: %.2f (valid: %d), Dist R: %.2f (valid: %d), Max Reach: %.2f", 
            ...
            if (!order_sent_) {
                ...
                if (distance_trigger && !vio_in_danger_) {
                    RCLCPP_WARN(this->get_logger(), "PREVENZIONE! Distanza superata (%.1fm). Ordine: %s", max_marker_interval_, order_msg.data.c_str());
                } else {
                    RCLCPP_WARN(this->get_logger(), "EMERGENZA VIO! Ordine inviato: %s", order_msg.data.c_str());
                }
            } else if ((this->now() - last_decision_time_).seconds() > 15.0) {
                order_sent_ = false;
            }
        } else {
            // Cooldown di sicurezza ridotto: 8 secondi coprono l'intera manovra (circa 6-7 sec)
            // più un paio di secondi di volo in avanti.
