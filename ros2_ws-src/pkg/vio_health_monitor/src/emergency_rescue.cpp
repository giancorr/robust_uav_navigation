#include "vio_health_monitor/emergency_rescue.hpp"
...
EmergencyRescueFSM::EmergencyRescueFSM() : Node("emergency_rescue_node"),
    ...
    sub_decision_ = this->create_subscription<std_msgs::msg::String>(
        ...
        [this](const std_msgs::msg::String::SharedPtr msg) {
            if (!enable_spray_) return; // Blocca i comandi di emergenza se disabilitato
            
            if (current_state_ == DroneState::NAVIGATING && msg->data != "NONE") {
                emergency_cmd_ = msg->data;
                RCLCPP_WARN(this->get_logger(), "ORDINE RICEVUTO: %s", emergency_cmd_.c_str());
            }
        });
...
void EmergencyRescueFSM::fsm_loop() {
    ...
    if (!initialized_) {
        ...
        RCLCPP_INFO(this->get_logger(), "Sensori allineati e PX4 pronto. INIZIO DECOLLO!");
    }
    ...
    switch (current_state_) {
        ...
        case DroneState::NAVIGATING:
            {
                ...
                if (!mission_started_) {
                    ...
                    if (dist < 0.5 || timeout) {
                        RCLCPP_INFO(this->get_logger(), "Obiettivo raggiunto! Inizio atterraggio.");
        ...
        case DroneState::DROP:
            {
                if (nav_step_ == 0) {
                    // Distanza di arresto cinematica per evitare di volare all'indietro per overshoot!
        ...
        case DroneState::STRAFE:
            {
                if (nav_step_ == 0) {
                    ...
                    // Distanza di arresto calcolata dinamicamente in base alla velocità
                    // Questo previene categoricamente il problema del drone che "supera" il punto e vola all'indietro!
                    double coast_dist = (cruise_speed_ * cruise_speed_) / (2.0 * acceleration_);
                    // Aggiungiamo un margine del 20% per sicurezza inerziale
                    coast_dist *= 1.2; 
                ...
                if (nav_step_ == 1) {
                    ...
                    if (impact_detected_ || timeout) {
                        ...
                        if (impact_detected_) {
                            RCLCPP_WARN(this->get_logger(), "MURO TOCCATO! INIZIO SWIPE.");
                            ...
                            RCLCPP_ERROR(this->get_logger(), "TIMEOUT STRAFE! Ritorno in rotta.");
        ...
        case DroneState::RECOVER:
            {
                if (nav_step_ == 0) {
                    // Proiettiamo la posizione attuale sulla linea del corridoio originale
                    // per calcolare il punto di rientro perfettamente perpendicolare!
        ...
        case DroneState::LANDING:
            {
                ...
                if (nav_step_ == 1 && (this->now() - maneuver_start_time_).seconds() > current_maneuver_time_) {
                    // Mantiene a terra
                    send_movement(current_x_, current_y_, 0.0, current_yaw_, -1.0); // -1.0 flag per stop
                    nav_step_ = 2;
                    RCLCPP_INFO(this->get_logger(), "Atterraggio completato.");
                }
            }
            break;
    }
