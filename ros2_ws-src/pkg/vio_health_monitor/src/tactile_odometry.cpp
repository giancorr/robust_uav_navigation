...
private:
    ...
    void init_transformations() {
        ...
        // Parametro per il thrust massimo (lo stesso usato nello stimatore)
    ...
    void thrust_callback(const px4_msgs::msg::VehicleThrustSetpoint::SharedPtr msg) {
        ...
        // Rotazione da FRD (PX4) a FLU (ROS)
    ...
    void init_ekf() {
        // Nessun EKF! Usiamo la pura geometria vettoriale e i vincoli unilaterali.
    }
    ...
    void odom_callback(const nav_msgs::msg::Odometry::SharedPtr msg) {
        ...
        // Ripristinato: la velocità è in ENU! La rotazione causava sbandate perché sovra-ruotava un vettore già globale.
        ...
        // La vera forza d'impatto deve essere ripulita dal bias (drag aerodinamico e mismatch di thrust)
        // altrimenti in volo veloce il drag aerodinamico o l'errore di thrust superano contact_threshold_
        // scatenando falsi impatti continui.
        Eigen::Vector3d true_impact_force = last_force_body_ - force_bias_;
        double force_norm = true_impact_force.norm();
        // Se la tactile odometry è disabilitata, forziamo is_impacting a false.
        ...
        if (!enable_tactile_) {
            ...
            // DISABILITIAMO LA Z: Il muro è fisico e verticale.
            // Qualsiasi componente Z nella forza è solo rumore (es. errore di stima del thrust).
            ...
            // --- PROIEZIONE CINEMATICA UNILATERALE ---
            ...
            // Controllo Posizione
            if (d_pen < 0.0) {
                // Penetrazione: proiettiamo ortogonalmente sulla superficie del muro
                hybrid_pos = current_pos_est - d_pen * wall_normal_enu_;
            } else {
                // Allontanamento o strisciamento esterno: liberi
                hybrid_pos = current_pos_est;
            }
            
            // Controllo Velocità
            if (v_pen < 0.0) {
                // Velocità verso l'interno: azzeriamo la componente perpendicolare al muro
                hybrid_vel = vel_meas - v_pen * wall_normal_enu_;
            } else {
                // Velocità verso l'esterno o parallela: liberi
                hybrid_vel = vel_meas;
            }
            
        } else {
            if (was_in_contact_) {
                // Transizione: Contatto -> Volo
                // Ricalcoliamo l'offset in modo che pos_meas + pos_offset si riagganci fluidamente all'ultima hybrid_pos
                pos_offset_ = last_hybrid_pos_ - pos_meas;
                was_in_contact_ = false;
                
                RCLCPP_INFO(this->get_logger(), "CONTACT ENDED. EKF Deactivated. New offset: (%.2f, %.2f, %.2f)", 
                            pos_offset_.x(), pos_offset_.y(), pos_offset_.z());
            }
            
            // Bypass EKF in volo libero, pass-through puro con offset per continuità
            hybrid_pos = pos_meas + pos_offset_;
            hybrid_vel = vel_meas; 
            
            // Aggiorniamo il bias a regime (filtro passa-basso) per assorbire l'errore del modello aerodinamico
            force_bias_ = 0.99 * force_bias_ + 0.01 * last_force_body_;
            
            // Impariamo il bias sull'accelerazione (ricostruita in modo filtrato).
    ...
    void publish_to_px4(const rclcpp::Time& stamp, const Eigen::Vector3d& pos_enu, const Eigen::Vector3d& vel_enu, const Eigen::Quaterniond& q_body_to_enu) {
        ...
        // vel_enu is ALREADY in World ENU. Just rotate to World NED.
