...
public:
    WrenchEstimatorNode() : Node("wrench_estimator_node")
    {
        ...
        this->declare_parameter<double>("max_torque_nm", 2.5);  // Coppia massima del drone [Nm]
        this->declare_parameter<double>("max_force_out", 50.0);  // Clamp forza in uscita [N]
        this->declare_parameter<double>("max_torque_out", 5.0);  // Clamp coppia in uscita [Nm]
...
private:
    ...
    // Matrice di rotazione da frame body PX4 (FRD) a frame body ROS (FLU)
    // R = [1  0  0;  0 -1  0;  0  0 -1]
    ...
    void imu_callback(const sensor_msgs::msg::Imu::SharedPtr msg)
    {
        ...
        // Controllo NaN completo su tutti e 6 i componenti IMU
        ...
        // Scarta letture fisicamente impossibili (Gazebo glitch durante collisioni)
        if (a.norm() > 500.0 || omega.norm() > 100.0) {
            RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                "IMU spike rilevato (a=%.1f, omega=%.1f), scartato.", a.norm(), omega.norm());
            return;
        }
        ...
        // Eq. (15): f_hat_e = integral( K_I_f * (M*a - f - f_hat_e) ) dt
        // L'IMU misura l'accelerazione propria (incluso il supporto contro la gravità).
        // Poiché ora la spinta è correttamente nel frame FLU, la gravità si cancella naturalmente.
        Eigen::Vector3d f_e_dot = K_I_f_ * ((mass_ * a) - current_thrust_ - f_e_hat_);
        f_e_hat_ += f_e_dot * dt;

        Eigen::Vector3d I_omega = inertia_ * omega;
        Eigen::Vector3d cross_term = I_omega.cross(omega);  // (I*omega) x omega
        
        // Eq. (15) corretta: integral_m = integral( m + (I*omega)×omega + m_hat_e ) dt
        // Segno + su m_hat_e per stabilità dell'osservatore (De Luca & Mattone):
        // dot(m_hat_e) = K_I_m * (m_ext - m_hat_e), autovalore -K_I_m < 0 → stabile
        ...
        // Clamp: limita a valori fisicamente plausibili
        ...
        // NaN/Inf Check completo su tutti e 3 i componenti — reset e NON pubblicare
        ...
        if (f_bad || m_bad) {
            ...
            p_omega_0_ = inertia_ * omega; // reset baseline
            return; // NON pubblicare valori corrotti
        }
