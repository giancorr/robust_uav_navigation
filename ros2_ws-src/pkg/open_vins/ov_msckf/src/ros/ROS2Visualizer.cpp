...
ROS2Visualizer::ROS2Visualizer(std::shared_ptr<rclcpp::Node> node, std::shared_ptr<VioManager> app, std::shared_ptr<Simulator> sim)
  ...
  // Publisher per i degeneracy factors (ROS 2 porting)
...
void ROS2Visualizer::publish_state() {
  ...
  // Publish degeneracy factor (ROS 2 porting)
  if (state != nullptr) {
      std_msgs::msg::Float32MultiArray msg_eigenvalue;
      std::vector<double> degen_factor = state->get_degen_factor();
      msg_eigenvalue.data.resize(degen_factor.size());
      for (size_t i = 0; i < degen_factor.size(); i++) {
          msg_eigenvalue.data[i] = static_cast<float>(degen_factor[i]);
      }
